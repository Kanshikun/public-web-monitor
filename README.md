...
公開Webの入口を、PCを起動していなくてもGitHub Actionsで確認するためのリポジトリです。公開前の準備段階です。remote作成・push・Actions実行・通知到達を終えるまで、稼働済みとは扱いません。
...
対象は`monitor.py`の固定allowlistだけです。認証なしGETを行い、Cookie・Authorization・環境変数proxyは使いません。redirectは追わず失敗にします。本文の読取上限は256 KiBで、本文・ヘッダー・例外の生テキストは表示しません。HTTPSの証明書検証は標準のままです。
...
workflowは毎時7・22・37・52分のUTCスケジュールと手動実行だけです。pushやPRでは自動実行しません。同時実行で進行中のジョブを取り消さず、tokenの権限は`contents: read`のみ、checkoutの認証情報も保存しません。
...
1. この内容を公開リポジトリのdefault branchへpushし、Actionsが許可されていることを確認する。
...
3. Actions → Public web monitor → Run workflowを、`simulate_failure=false`で実行する。両対象のPASSとJob summaryのUTC日時を確認する。
